...
transform_visual_selector tag widget visual_selector=case visual_selector of
    ...
    Any_visual_selector {value,strict}->Combine_visual_selector {combine_visual_selector=DS.fromList (map (\index->Vector_visual_selector {vector_value=DIM.singleton index value,bounded=True,strict=strict}) (get_active_indices tag widget))}
    _->visual_selector
...
maybe_update_collect_extension_widget tag maybe_border projection_path leaf_id selector visual_selector collect_strategy engine=case DFC.getCompose (functor_lookup_projection_widget projection_path (\widget->DFC.Compose {getCompose=fmap (\this_widget->(to_collect (transform_visual_selector tag this_widget visual_selector) engine.u engine.v maybe_border (get_view tag this_widget),this_widget)) (selector_monad_update (const (get_update tag)) selector widget)}) engine) of
    Nothing->engine
...
maybe_collect_update_extension_widget tag maybe_border projection_path leaf_id selector visual_selector collect_strategy engine=let (update,maybe_engine)=DFC.getCompose (functor_lookup_projection_widget projection_path (\widget->DFC.Compose {getCompose=(int_map_update leaf_id (update_projection_object (collect_a (to_collect (transform_visual_selector tag widget visual_selector) engine.u engine.v maybe_border (get_view tag widget)) collect_strategy)),selector_monad_update (const (get_update tag)) selector widget)}) engine) in case maybe_engine of
    Nothing->engine
...
collect_extension_widget tag maybe_border projection_path leaf_id selector visual_selector collect_strategy engine=let widget=lookup_projection_widget projection_path engine in engine {leaf=int_map_update leaf_id (update_projection_object (selector_update (const (collect_a (to_collect (transform_visual_selector tag widget visual_selector) engine.u engine.v maybe_border (get_view tag widget)) collect_strategy)) selector)) engine.leaf}
