...
page_visual_trigger event _ visual=case event of
    At {window_id=this_window_id,action}->case visual of
        Custom_visual {custom}->case custom of
            Page {window_id,arrange,x,y,half_width,half_height,step_size,hovered,pressed,text,strict_exist,strict_match}->if this_window_id==window_id
                then case action of
                    Press {press,change}->case press of
                        Press_down->if pressed
                            then case change of
                                Key_down->(Custom_visual {custom=update_page_state strict_match True hovered pressed (scroll_text strict_match step_size text) custom},id)
                                Key_up->(Custom_visual {custom=update_page_state strict_match True hovered pressed (scroll_text strict_match (negate step_size) text) custom},id)
                                Key_page_down->(Custom_visual {custom=update_page_state strict_match True hovered pressed (scroll_bottom_text strict_match text) custom},id)
                                Key_page_up->(Custom_visual {custom=update_page_state strict_match True hovered pressed (scroll_top_text strict_match text) custom},id)
                                _->(visual,id)
                            else (visual,id)
                        Press_up->(visual,id)
                    Click {press,mouse_button,x=click_x,y=click_y}->case mouse_button of
                        Mouse_button_left->case press of
                            Press_down->let above=above_extension_box click_x click_y arrange x y half_width half_height in if above/=pressed then (Custom_visual {custom=update_page_state strict_match True hovered above text custom},id) else (visual,id)
                            Press_up->(visual,id)
                        Mouse_button_right->case press of
                            Press_down->let above=above_extension_box click_x click_y arrange x y half_width half_height in if above&&pressed then (Custom_visual {custom=update_page_state strict_match True hovered False text custom},id) else (visual,id)
                            Press_up->(visual,id)
                        _->(visual,id)
                    Move {x=move_x,y=move_y}->let above=above_extension_box move_x move_y arrange x y half_width half_height in if above/=hovered then (Custom_visual {custom=update_page_state strict_match True above pressed text custom},if above then \this_engine->this_engine {request=this_engine.request DS.|> Set_system_cursor {system_cursor=System_cursor_pointer,strict_exist=strict_exist}} else \this_engine->this_engine {request=this_engine.request DS.|> Set_system_cursor {system_cursor=System_cursor_default,strict_exist=strict_exist}}) else (visual,id)
                    Scroll {x=scroll_x,y=scroll_y,delta_y}->if pressed&&above_extension_box scroll_x scroll_y arrange x y half_width half_height then (Custom_visual {custom=update_page_state strict_match True hovered pressed (scroll_text strict_match (negate delta_y*step_size) text) custom},id) else (visual,id)
                    _->(visual,id)
                else (visual,id)
            _->(visual,id)
        _->(visual,id)
