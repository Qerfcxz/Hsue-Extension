...
scroll_page transform widget=case widget of
    Vector {vector_widget}->case vector_widget DV.! extension_page_visual_index of
        Vector_visual {arrange,collect_order,vector_visual,size}->let (new_text_visual,changed)=update_text transform (vector_visual DV.! extension_page_text_visual_index) in if changed then update_vector_bool (const True) extension_page_dirty_index (update_vector_widget extension_page_visual_index (const (Vector_visual {arrange=arrange,collect_order=collect_order,vector_visual=DV.modify (\this_vector_visual->DVM.write this_vector_visual extension_page_text_visual_index new_text_visual) vector_visual,size=size})) widget) else widget
        _->EF.empty_error
    _->EF.empty_error

create_page_request::ET.Has_call_stack=>(Event a->Engine a->Maybe Int)->(Event a->Engine a->Widget a->(Widget a,Engine a->Engine a))->Extension_widget_request a->Widget_request a
