...
create_page_visual page_request engine=case page_request of
    Page_request {window_id,arrange,x,y,page_width,page_height,inner_thickness,outer_thickness,step_size,calculate_width,calculate_typesetting,anchor,article,load,color,outer_color,hovered_color,outer_hovered_color,selected_color,outer_selected_color,hovered_selected_color,outer_hovered_selected_color}->let half_width=page_width/2 in let half_height=page_height/2 in let text_width=max 0 (page_width-2*inner_thickness) in let text_height=max 0 (page_height-2*inner_thickness) in do
        (new_engine,text_visual)<-create_visual (Text_request {arrange=default_arrange {point=Point {x=x,y=y}},text_width=text_width,text_height=text_height,calculate_width=calculate_width,calculate_typesetting=calculate_typesetting,anchor=anchor,article=article,load=load}) engine
...
update_page_state::ET.Has_call_stack=>Bool->Bool->Bool->Visual a->Extension_visual a->Extension_visual a
update_page_state dirty hovered pressed text page=case page of
                ...
                then case action of
                    Press {press,change}->case press of
                        Press_down->if pressed
                            then case change of
                                Key_down->(Custom_visual {custom=update_page_state True hovered pressed (scroll_text step_size text) custom},id)
                                Key_up->(Custom_visual {custom=update_page_state True hovered pressed (scroll_text (negate step_size) text) custom},id)
                                Key_page_down->(Custom_visual {custom=update_page_state True hovered pressed (scroll_bottom_text text) custom},id)
                                Key_page_up->(Custom_visual {custom=update_page_state True hovered pressed (scroll_top_text text) custom},id)
                                _->(visual,id)
                            else (visual,id)
                        _->(visual,id)
                    Click {press,mouse_button,x=click_x,y=click_y}->case mouse_button of
                        Mouse_button_left->case press of
                            Press_down->let above=above_extension_box click_x click_y arrange x y half_width half_height in if above/=pressed then (Custom_visual {custom=update_page_state True hovered above text custom},id) else (visual,id)
                            _->(visual,id)
                        Mouse_button_right->case press of
                            Press_down->let above=above_extension_box click_x click_y arrange x y half_width half_height in if above&&pressed then (Custom_visual {custom=update_page_state True hovered False text custom},id) else (visual,id)
                            _->(visual,id)
                        _->(visual,id)
                    ...
                    Scroll {x=scroll_x,y=scroll_y,delta_y}->if pressed&&above_extension_box scroll_x scroll_y arrange x y half_width half_height then (Custom_visual {custom=update_page_state True hovered pressed (scroll_text (negate delta_y*step_size) text) custom},id) else (visual,id)
                    _->(visual,id)
                else (visual,id)
            _->(visual,id)
        _->(visual,id)
    ...
    Page {arrange,x,y,half_width,half_height,inner_thickness,outer_thickness,hovered,pressed,color,outer_color,hovered_color,outer_hovered_color,selected_color,outer_selected_color,hovered_selected_color,outer_hovered_selected_color,text}->case text of
        Text {arrange=text_arrange,current_y,anchor,article}->case transform arrange of
            Arrange {point=new_point,matrix=new_matrix,color=new_color}->case text_arrange of
                Arrange {matrix=text_matrix,color=text_color}->let current_color=get_page_color hovered pressed color hovered_color selected_color hovered_selected_color in
                    let current_outer_color=get_page_color hovered pressed outer_color outer_hovered_color outer_selected_color outer_hovered_selected_color in
                    let inner_half_width=half_width in
                    let inner_half_height=half_height in
                    ...
                    let inner_submit=create_submit_rectangle Submit_default maybe_border inner_arrange inner_half_width inner_half_height u v u v in
                    let text_submit=create_submit_text Submit_default maybe_border text_combined_arrange (inner_half_width-inner_thickness) (inner_half_height-inner_thickness) current_y anchor article in
...
page_setter::ET.Has_call_stack=>Custom_extension a=>FCT.CFloat->FCT.CFloat->Maybe (Widget a->Widget a)
page_setter cached_offset offset=if cached_offset==offset then Nothing else Just (page_setter_a offset)

page_setter_a::ET.Has_call_stack=>Custom_extension a=>FCT.CFloat->Widget a->Widget a
page_setter_a offset widget=case widget of
