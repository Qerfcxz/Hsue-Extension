...
button_widget_trigger this_action event _ widget=case event of
    At {window_id,action}->case widget of
        Vector {vector_widget}->if window_id==get_store_widget (vector_widget DV.! extension_button_window_id_index) then case action of
            Click {press,mouse_button,x,y}->case mouse_button of
                Mouse_button_left->case press of
                    Press_up->if view_button_bool widget extension_button_pressed_index then let new_widget=update_button_bool (const True) extension_button_dirty_index (update_button_bool (const False) extension_button_pressed_index widget) in if above_button x y (vector_widget DV.! extension_button_visual_index) then (new_widget,this_action) else (new_widget,id) else (widget,id)
                    Press_down->if above_button x y (vector_widget DV.! extension_button_visual_index) then (update_button_bool (const True) extension_button_dirty_index (update_button_bool (const True) extension_button_pressed_index widget),id) else (widget,id)
                _->(widget,id)
            Move {x,y}->let above=above_button x y (vector_widget DV.! extension_button_visual_index) in if above/=view_button_bool widget extension_button_hovered_index then (update_button_bool (const True) extension_button_dirty_index (update_button_bool (const above) extension_button_hovered_index widget),if above then \engine->engine {request=engine.request DS.|> Set_system_cursor {system_cursor=System_cursor_pointer}} else \engine->engine {request=engine.request DS.|> Set_system_cursor {system_cursor=System_cursor_default}}) else (widget,id)
            _->(widget,id)
        else (widget,id)
...
above_button x y widget=case widget of
    Vector_visual {arrange=first_arrange,vector_visual}->case vector_visual DV.! extension_button_inner_rectangle_index of
...
update_button this_widget=case this_widget of
    Widget_trigger {next,widget_trigger,widget}->case widget of
        Vector {vector_widget}->if get_store_widget (vector_widget DV.! extension_button_dirty_index) then Just (Widget_trigger {next=next,widget_trigger=widget_trigger,widget=update_button_bool (const False) extension_button_dirty_index widget}) else Nothing
...
{-# INLINE maybe_update_collect_button #-}
{-# INLINE maybe_collect_update_button #-}
{-# INLINE collect_button #-}